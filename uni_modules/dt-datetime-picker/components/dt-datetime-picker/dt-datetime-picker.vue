<template>
  <view class="datetime-picker">
    <view class="datetime-picker__mask" v-if="visible" @click="handleCancel" @touchmove.stop.prevent="() => { }"></view>
    <view class="datetime-picker__container" :class="{ 'datetime-picker__container--show': visible }">
      ...
      <view v-if="quickOptions.length > 0" class="datetime-picker__quick">
        <text v-for="(option, index) in quickOptions" :key="index" class="datetime-picker__quick-item"
          :class="{ 'datetime-picker__quick-item--active': currentQuickIndex === index }"
          @click="handleQuickSelect(option, index)">{{ option.label }}</text>
      </view>

      <!-- 区间选择标签 -->
      <view v-if="isRange" class="datetime-picker__range-tabs">
        <text class="datetime-picker__range-tab" :class="{ 'datetime-picker__range-tab--active': rangeIndex === 0 }"
          @click="handleRangeChange(0)">开始时间</text>
        <text class="datetime-picker__range-tab" :class="{ 'datetime-picker__range-tab--active': rangeIndex === 1 }"
          @click="handleRangeChange(1)">结束时间</text>
      </view>

      <view class="datetime-picker__body">
        <picker-view :value="pickerValue" @change="handleChange" class="datetime-picker__picker"
          :indicator-style="indicatorStyle" :mask-style="maskStyle" :style="{ height: `${height}px` }">
          <picker-view-column v-if="showYear">
            <view class="datetime-picker__item" v-for="year in years" :key="year">{{ year }}年</view>
          </picker-view-column>
          <picker-view-column v-if="showMonth">
            <view class="datetime-picker__item" v-for="month in months" :key="month">{{ month }}月</view>
          </picker-view-column>
          <picker-view-column v-if="showDay">
            <view class="datetime-picker__item" v-for="day in days" :key="day">{{ day }}日</view>
          </picker-view-column>
          <picker-view-column v-if="showHour">
            <view class="datetime-picker__item" v-for="hour in hours" :key="hour">{{ hour }}时</view>
          </picker-view-column>
          <picker-view-column v-if="showMinute">
            <view class="datetime-picker__item" v-for="minute in minutes" :key="minute">{{ minute }}分</view>
          </picker-view-column>
          <picker-view-column v-if="showSecond">
            <view class="datetime-picker__item" v-for="second in seconds" :key="second">{{ second }}秒</view>
          </picker-view-column>
        </picker-view>
      </view>
    </view>
